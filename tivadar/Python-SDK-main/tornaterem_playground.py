...
from simulation import playground
import time
import keyboard
from multiprocessing import Process, Queue, Manager
from simulation.playground_util import PlaygroundDataOperator
...
def playground_process_main(operator):
    pg = playground.Playground(operator)
    pg.prepare()
    pg.run_with_agent()
...
class MistyEnv(gym.Env):
    def __init__(self):
        ...
        self.saw_the_target = False
        self.stopped = False
        self.data_container = DataContainer(8)
        self.reward_logger = RewardLogger()

        self.manager = Manager()
        self.playground_operator = PlaygroundDataOperator(self.manager)
        self.playground_process = Process(
            target=playground_process_main,
            args=(self.playground_operator,),
            daemon=True
        )
        self.playground_process.start()
    ...
    def run_playground_process(self):
        pg = playground.Playground(
            self.playground_operator
        )
        pg.prepare()
        pg.run_with_agent()
    ...
    def step(self, action):
        ...
        fw = float(action[0])
        yaw = float(action[1])
        self.playground_operator.set_robot_input(fw, yaw)
        ...
        if truncated:
            ...
            self.playground_operator.command["stop"] = True
        if terminated:
            print('terminated, wasd esc')
            self.reward_logger.next_round()
            print(self.reward_logger.to_string_next_round())
            self.playground_operator.command["reset"] = True
            self.game_step_counter = 0
            self.saw_the_target = False
            dp = DataPocket()
            for i in range(self.data_container.size):
                self.data_container.push(dp)
    ...
    def get_current_observation(self, fw, yaw, reward):
        robot_data = self.playground_operator.get_robot_data()
        target_pos = robot_data.target_pos_in_view
        visible = 1
        if not robot_data.target_visible:
            target_pos = 0
            visible = 0
        stopped_number = 0
        if self.stopped:
        ...
        data_pocket = DataPocket(x_diff=target_pos,
                                 visible=visible,
                                 d_left= robot_data.d_fl,
                                 d_center= robot_data.d_fc,
                                 d_right= robot_data.d_fr,
                                 d_back= robot_data.d_bc,
                                 b_fl = robot_data.b_fl,
                                 b_fr = robot_data.b_fr,
                                 b_bl = robot_data.b_bl,
                                 b_br = robot_data.b_br,
                                 linear_v = fw,
                                 angular_v = yaw,
                                 reward = reward,)
        self.data_container.push(data_pocket)
        obs = self.data_container.to_list()
        print(f"time: {time.time() - robot_data.timestamp}, gamestep: {self.game_step_counter}")
        print(data_pocket.to_string())
    ...
    def request_reward(self,fw,yaw):
        ...
        if not robot_data.target_visible:
            ...
            x_diff_reward = (1 - abs(robot_data.target_pos_in_view)) + 0.5
            # fw_reward = fw * 0.5

        if robot_data.b_fl or robot_data.b_fr or robot_data.b_bl or robot_data.b_br:
            bump_reward = -4
        if robot_data.target_visible:
            distance_reward = ((1 - robot_data.d_fc) ** 2) * 2
            self.saw_the_target = True
        if self.saw_the_target and not robot_data.target_visible:
            x_diff_reward = -0.1
        reward = x_diff_reward + distance_reward + fw_reward + yaw_reward + bump_reward #- self.game_step_counter * 0.005
        return float(reward)

    def is_terminated(self):
        robot_data = self.playground_operator.get_robot_data()
        if robot_data.b_fl or robot_data.b_fr or robot_data.b_bl or robot_data.b_br or self.game_step_counter > 200:
            return True
        return False

    def close(self):
        print("closing env")
        if self.playground_process.is_alive():
            self.playground_process.terminate()
            self.playground_process.join()
